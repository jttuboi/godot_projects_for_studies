...
void vertex() {
	...
	if (RESTART || restart) {
		...
		// CONVERT PARTICLES IN COLOR OF SPRITE, GIVING A SHAPE TOO
		vec2 particlePosition = TRANSFORM[3].xy;
		vec2 textureS = vec2(textureSize(sprite, 0));
		vec4 spriteColor = texture(sprite, particlePosition / textureS + vec2(0.5, 0.5)); // the vec2(0.5, 0.5) redirect the particles to center
		COLOR = spriteColor;
		
		// initial_linear_velocity WAS SET BY 1
		// CHANGE VELOCITY OF THIS PARTICLES USING initial_linear_velocity
		// THE EFFECT IS GOING FAR FROM CENTER
		// THE EFFECT IS GOING FAR FROM CENTER
		VELOCITY.xy = particlePosition * initial_linear_velocity;
		
		// Time/explosiveness GIVE THE EFFECT OF EXPLOSION
		
		// REMOVE INVISIBLE PARTICLES BECAUSE IT'S USELESS IF CAN'T SEE IT
...
//	COLOR = hue_rot_mat * color_value;
	...
	// CHANGE THE TRANSPARENCY OF PARTICLES BY TIME
	// WHEN MORE OLD THE LIFETIEM PARTICLE, MORE TRANSPARENT BE IT
	if (COLOR.a > 0.0) {
