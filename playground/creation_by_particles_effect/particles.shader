...
void vertex() {
	...
	bool restart = false;
	float tv = 0.0;
	if (CUSTOM.y > LIFETIME) {//	if (CUSTOM.y > CUSTOM.w) {
		restart = true;
		tv = 1.0;
	}
...
//		CUSTOM.z = (anim_offset + tex_anim_offset) * mix(1.0, anim_offset_rand, anim_offset_random);
		...
		// CALCULATE FIRST POSITIONS OF PARTICLES
		...
		vec2 spriteUV = targetPosition / textureS + vec2(0.4999, 0.4999); // using 0.4999 fix the problem of pixels going to wrong position becuase of even number of columns
		vec4 spriteColor = texture(sprite, spriteUV);
		COLOR = spriteColor;
		if (spriteColor.a == 0.0) { // IGNORE INVISIBLE PARTICLES AVOID RENDERING THEM
			ACTIVE = false;
		}
		
		targetPosition -= vec2(0.5, 0.5);
		
		// SET THE VELOCITY OF PARTICLES
...
//	COLOR = hue_rot_mat * color_value;
	...
	// CONTROL THE MOMENTUM OF PARTICLES
	// STOP THE MOMENTUM WHEN THE LIFETIME (STORED IN CUSTOM.y) IS TIME TO STOP AFTER 1s
	float stopTime = 1.0 / initial_linear_velocity; 
	if (CUSTOM.y > stopTime && COLOR.a > 0.0) {
		VELOCITY.xy = vec2(0, 0);
		TRANSFORM[3].xy = CUSTOM.zw;
		// GIVE SMOOTHY TRANSPARENCY AFTER PARTICLES STAY IN END POSITION
		// START WITH THE CURRENT ALPHA AND STOP TO ZERO
		COLOR.a = max(COLOR.a - (DELTA * 2.0) / (LIFETIME - stopTime), 0.0);
	} else {
		// GIVE FADE IN EFFECT IN PARTICLES IN THE BEGINNING OF LIFETIME
